...
func main() {
	...
	if ceroElapsed < uuidElapsed {
		...
		fmt.Printf("  latency avg: cero-id is %.2fx lower (%.1f vs %.1f ns)\n",
			uuidHist.Mean()/ceroHist.Mean(), ceroHist.Mean(), uuidHist.Mean())
		...
		fmt.Printf("  latency avg: UUIDv7 is %.2fx lower (%.1f vs %.1f ns)\n",
			ceroHist.Mean()/uuidHist.Mean(), uuidHist.Mean(), ceroHist.Mean())
