...
// Intended for testing and single-node deployments with manually assigned IDs.
func Static(workerID uint8) Registry {
	return &staticRegistry{id: workerID}
}

type staticRegistry struct{ id uint8 }

func (s *staticRegistry) Claim(_ context.Context, _ uint8, _ bool) (uint8, func() error, error) {
	if s.id > 127 {
		return 0, nil, fmt.Errorf("registry: static worker ID %d exceeds max 127", s.id)
	}
